...
function Header() {
    ...
    const ReceivePrice = () => {
        ...
        let getPrice = document.querySelectorAll('.price-text');
       let nodes = Array.prototype.slice.call(getPrice,0);
        console.log(nodes)
        nodes.forEach(function(node){ 
            console.log(node.innerHTML, 'inside foreach')
            
            let newValue = parseFloat(node.innerHTML.replace(",", "."));
            priceChange.push(newValue)
            
       });
       
       var priceTotal = priceChange.reduce((a, b) => a + b, 0) 
         
       setReceiveP(priceTotal.toLocaleString('pt-br', {minimumFractionDigits: 2}))
    }
        ...
        <div className="nav">
            <div className="container">
              ...
              <div className="fix-center size-10 size-mob-10">
                <div className={`size-mob-2 ${btnMobile ? "hidden" : ""} ${view > 767 ? "hidden" : ""}`} onClick={() => openMenuMob(true)}>
                    ...
                    <div className="cart">
                        ...
                        {numClick > 0 ? 
                            <div className="cart-item">
                                    ...
                                    <div className="cart-centralize">
                                        {linkar.map(item => {
                                            return (  
                                                <div>
                                                    <div className="card">
                                                        <div className="image-product">
                                                        <img src={item.event.imageUrl} alt={item.event.productName} />
                                                        </div>
                                                        <div className="product-info">
                                                            <h4>{item.event.productName}</h4>
                                                            <div className="price">
                                                                <span className="money-pt-br">R$</span><span id="test-price-take" className="price-text">{item.event.price.toLocaleString('pt-br', {minimumFractionDigits: 2})}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                ...
                <div className={`close-menu-mobile ${!btnMobile ? "hidden" : ""}`} onClick={() => closeMenuMob(false)}>
                    <span className="line line-1"></span>
                    <span className="line line-2"></span>
                </div>
              </div>
